#!/usr/bin/env bash
#
# Renders tx3c's client for one language with `tx3c codegen --language` and
# compiles the result against the published SDK release its templates pin.
#
...
#
# Usage: codegen-compile-check.sh <rust|typescript|python|go> [path/to/tx3c]
...
templates="$repo_root/bin/tx3c/templates/$language"
work="$(mktemp -d)"
...
for fixture in transfer complex; do
  gen="$work/$fixture"
  "$tx3c" codegen --tii "$fixtures/$fixture.tii" --language "$language" --output "$gen"
  echo "--- $language/$fixture"
  ...
  case "$language" in
    ...
    typescript)
      ...
      range="$(sed -n 's/.*"tx3-sdk": "\(.*\)".*/\1/p' "$templates/package.json.hbs")"
      (
        cd "$gen"
        npm init -y >/dev/null
...
echo "codegen compile check passed for $language"
